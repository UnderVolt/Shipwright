...
void FileChoose_DrawTexture(GameState* thisx, s32 x, s32 y, u32 w, u32 h, u8* texture) {
    FileChooseContext* this = (FileChooseContext*)thisx;

    OPEN_DISPS(this->state.gfxCtx);
    gDPPipeSync(POLY_OPA_DISP++);
    gDPLoadTextureBlock(POLY_OPA_DISP++, texture, G_IM_FMT_IA, G_IM_SIZ_16b, 44, 16, 0,
                        G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMASK, G_TX_NOMASK, G_TX_NOLOD,
                        G_TX_NOLOD);
    gDPSetTextureImage(POLY_OPA_DISP++, G_IM_FMT_IA, G_IM_SIZ_16b, 1, texture);
    gSPTextureRectangle(POLY_OPA_DISP++, x << 2, y << 2, (x + w) << 2, (y + h) << 2, G_TX_RENDERTILE, 0, 0, 1 << 10,
                        1 << 10);
    CLOSE_DISPS(this->state.gfxCtx);
}
...
void FileChoose_DrawWindowContents(GameState* thisx) {
    ...
    for (i = 0; i < 3; i++, temp += 20) {
        ...
        if (isRandSave || isMQSave) {
            gDPSetPrimColor(POLY_OPA_DISP++, 0, 0, color.r, color.g, color.b, this->nameAlpha[i]);
        ...
        if (isRandSave || isCloudSave || isMQSave) {
...
void FileChoose_ConfigModeDraw(GameState* thisx) {
    ...
    HMClient_SetEditEnabled(this->configMode == CM_MAIN_MENU);
